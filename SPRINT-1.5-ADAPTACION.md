...
# Parte 2 — Detail scraper (uno por uno)

Este es el segundo paso del flujo: una vez que `scrape:listing` pobló la DB con los cards, `scrape:detail` visita cada remate pendiente y extrae los datos completos.

## Qué hace

```
Para cada remate con detail_scraped_at = NULL:
  1. Abre contexto Playwright fresco
  2. Va al buscador → escribe el número → captcha "x" → click APLICAR
  3. Click Detalle → parsea 3 pestañas:
     - Pestaña Remate  → datos económicos y legales
     - Pestaña Inmuebles → cada inmueble con cargas
     - Pestaña Cronograma → 5 fases con fechas
  4. UPSERT transaccional en DB
  5. Cierra contexto
  6. Espera 3s (rate limit)
```

## Estructura de archivos

```
apps/scraper/src/detail/
├── parsers/
│   ├── cargas.ts          ← Detecta hipoteca/embargo desde texto crudo
│   ├── tab-remate.ts      ← Parsea tasación, precio_base, expediente, etc.
│   ├── tab-inmuebles.ts   ← Parsea la tabla de inmuebles (puede ser N filas)
│   └── tab-cronograma.ts  ← Parsea las 5 fases con fechas
├── __tests__/
│   └── detail-parsers.test.ts  ← Tests — necesitan HTML real (ver más abajo)
└── scrape-detail.ts       ← Orquestador (reemplaza el esqueleto de deliverables)
```
...
```bash
cp -r sprint-1.5/apps/scraper/src/detail/ /tu/repo/apps/scraper/src/
```

### Paso 2 — Conectar la DB

En `scrape-detail.ts`, descomentá los imports y el bloque de Drizzle en `selectPendingRemates` y `persistDetail`. Es el mismo patrón que hiciste con el listing.

### Paso 3 — Capturar HTML real para validar selectores

Los parsers usan selectores **estimados** basados en PrimeFaces. Antes de usar en prod, capturá el HTML real:

En `scrape-detail.ts`, dentro de `scrapeRemateDetail`, agregá temporalmente después de parsear:
...
Corré con un solo remate:

```bash
pnpm scrape:detail --remate 23430
```

### Paso 4 — Ajustar selectores con los tests

Pegá el HTML capturado en los fixtures de `__tests__/detail-parsers.test.ts` y corré:

```bash
pnpm test detail-parsers
```

Los tests te dicen exactamente qué campo no se está parseando. Ajustás los selectores en el parser correspondiente hasta que todos pasen.

### Paso 5 — Soltar el batch completo

Una vez que los tests pasan con HTML real:

```bash
# Primero, máximo 10 para validar
pnpm scrape:detail --limit 10

# Verificar en DB
sqlite3 remaju.db "SELECT remate_numero, tasacion, precio_base, convocatoria, fecha_fin_inscripcion FROM remates WHERE detail_scraped_at IS NOT NULL LIMIT 5;"
...
```bash
# Correr 2 veces el mismo remate no debería duplicar inmuebles
pnpm scrape:detail --remate 23430 --force
pnpm scrape:detail --remate 23430 --force

sqlite3 remaju.db "SELECT COUNT(*) FROM remate_inmuebles WHERE remate_id = (SELECT id FROM remates WHERE remate_numero = '23430');"
# → debe dar el mismo número las 2 veces
```
...
**Listing scraper (Parte 1):**
✅ Parser completo del card (23 tests con HTML real)
✅ UPSERT idempotente — corré N veces sin destruir nada
✅ Archivado soft-delete con pre-condición de seguridad
✅ TypeScript compila sin errores

**Detail scraper (Parte 2):**
✅ Orquestador completo con retries y rate limiting
✅ Parser tab-remate — datos económicos y legales
✅ Parser tab-inmuebles — tabla de inmuebles con cargas
✅ Parser tab-cronograma — 5 fases con fechas críticas para scoring
✅ Tests estructurados — solo necesitan el HTML real para completar

⚠️ **Lo que tenés que hacer vos en ambas partes:**
- Conectar los imports de `db` y `remates` a tu Drizzle real
- Capturar HTML real del portal para validar selectores del detail
- Adaptar selectores del listing a lo que ya tenías funcionando
- Correr los tests hasta que pasen con HTML real

**Tiempo estimado de adaptación total: 3-6 horas.**

Cuando tengas dudas con un selector o una query, abrime un chat con el HTML/error concreto y te lo resuelvo.
